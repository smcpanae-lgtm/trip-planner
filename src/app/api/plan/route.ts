...
- **犬が入場できない施設**（神社仏閣の境内、一部テーマパーク等）がユーザー指定の目的地に含まれる場合:
  - プランA: その目的地を含めたプランを作成し、descriptionに「⚠️ ペット入場不可の施設です。車内で待機させるか、近隣のペット預かり施設の利用をご検討ください」と明記する
  - プランB: 犬が入場できない目的地の代わりに、近隣の犬同伴可能な代替スポットを提案する。removedSpotsに元の目的地と「ペット入場不可のため代替スポットを提案」という理由を記載する
  - tipsに犬が入場不可の施設についての注意事項と対策を含める`
    : "";
...
**重要: ユーザーが具体的に指定した目的地がある場合は、プランA・プランBの両方に必ずその目的地を含めてください。おまかせ部分のみ異なるテーマで提案してください。**
- **プランA**: 定番・王道の観光スポットを中心としたプラン（ユーザー指定の目的地+定番スポット）
- **プランB**: 穴場・ユニークな体験を重視したプラン（ユーザー指定の目的地+穴場スポット）
それぞれのplanNameとplanDescriptionで、プランの特徴やテーマの違いを明確に説明してください。`;
  } else {
    planVariationInstruction = `
## 2プラン作成（目的地指定モード）
**重要: プランA・プランBの両方に、ユーザーが指定した目的地をすべて必ず含めてください。指定された目的地を省略・除外・代替することは禁止です。**
- **プランA「効率プラン」**: ユーザー指定の目的地のみで構成。無駄のない効率的なルート
- **プランB「充実プラン」**: ユーザー指定の目的地を**すべて含めた上で**、さらにルート上で立ち寄れるおすすめスポットをAIが追加提案。空き時間を有効活用
プランBはプランAの目的地を全て含み、それに追加スポットを足したプランです。プランAの目的地を削って別のスポットに入れ替えることは絶対にしないでください。
...
9. 昼食・夕食について:
   - 昼食の希望ジャンルが記載されている場合: 11:30〜13:30の時間帯にitemsの中にtype="lunch"の食事スポットを**必ず1件**追加すること（具体的な店名と滞在時間60分）。これはルート上の実在する店舗でなければならない
   - 夕食の希望ジャンルが記載されている場合: 17:30〜19:30の時間帯にitemsの中にtype="dinner"の食事スポットを**必ず1件**追加すること（具体的な店名と滞在時間60分）。これはルート上の実在する店舗でなければならない
   - 食事スポットには必ずlat/lng/addressを含めること（ルート上の地図に表示するため）
...
11. **食事のおすすめ店舗リスト**:
   - 昼食・夕食のジャンルが指定されている場合、itemsに追加した1件に加えて、lunchSpot/dinnerSpotの中に**おすすめ店舗を3〜4件**リストアップすること
   - 各店舗は「店名 - 特徴（ジャンル、人気メニュー等）」の形式で記載
   - 犬連れ旅行の場合: テラス席あり・犬同伴可の店舗を優先し、「🐕テラス席犬OK」「🐕店内犬同伴可」等を明記すること。犬同伴不可の店舗は候補に含めないこと
   - nearSpotには「○○（目的地名）から車で約○分」のようにアクセス情報を記載
